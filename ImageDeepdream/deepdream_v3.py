...
def apply_deep_dream(proc_image, camera_image):
    ...
    if proc_image is not None:
        ...
        proc_image = proc_image * (1.0 - image_blend_factor) + camera_image * image_blend_factor
...
camera = setup_video_capture(0, (1024, 576))
...
for root, _, fnames in sorted(os.walk(images_file_path, followlinks=True)):
    ...
    for fname in fnames:
        ...
        image_full_path = images_file_path + "/" + fname
        
        file_img = load_image(image_full_path, target_shape=config['img_width'])
        
        if proc_img is not None:
            file_img = file_img * image_blend_factor + proc_img * (1.0 - image_blend_factor)

        proc_img = deep_dream_static_image(config, file_img)  # yep a single liner
        
        dump_path = "results/images/" + fname
        
        config["input"] = images_file_path + "/" + fname
        
        config['should_display'] = False
        dump_path = save_and_maybe_display_image(config, proc_img)
        #print(f'Saved DeepDream static image to: {os.path.relpath(dump_path)}\n')
